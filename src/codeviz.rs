...
pub fn print_code_warn(code_warn: CodeWarning, file_manager: &FileManager) {
    ...
    snip = snip.annotation(
        match code_warn.pointer {
            None => {
                Level::Warning.span(code_warn.position.range(offset))
            }
            Some(_) => {
                Level::Warning.span(code_warn.position.range(offset)).label(code_warn.pointer.unwrap().leak())
            }
        }
    );
    ...
    let msg = Level::Warning.title(code_warn.title.as_str()).id(&*id_fmt).snippet(snip).footers(footers);
