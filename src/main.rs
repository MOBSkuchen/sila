...
fn main() {
    ...
    argument_parser.add_argument(
        Argument::new("compile".to_string(), vec!["file_path".to_string()],
                      mk_clfn!(_compile), 
                      "Compile a file".to_string(), false));
    argument_parser.add_flag(
        Flag::new("--output".to_string(), "-o".to_string(), true, empty!(), 
    ...
    for pending_call in pending_calls {
        if pending_call.has_name("compile".to_string()) {
            pending_call.call(&argument_parser, Some(&pending_call.merge_args(vec![(&flag_map).get("--output")
